...
# Arnott Ramos Caiado
# maio de 2024
# preparado para feira do empreendedor e codigos RFID e NFC
# fonte:
# https://www.youtube.com/watch?v=VNaTl2i5P1U
# https://www.youtube.com/watch?v=WDpPGFkI9UU
#
# rota principal
# https://iotmaker.pythonanywhere.com/
#
...
arq='/home/iotmaker/mysite/arquivos/log.csv'
...
# criação da classe com a estrutura da tabela Usuarios
class Usuario(db.Model):
    ...
    def to_json(self):
        return {"id": self.id, "nome": self.nome, "email": self.email, "senha": self.senha, "token": self.token}
...
class Tag(db.Model):
    ...
    def to_json(self):
        return {"id": self.id, "tag": self.tag, "descricao": self.descricao, "tipo": self.tipo}

class Requisicao(db.Model):
    id = db.Column(db.Integer, primary_key=True )
    data = db.Column( db.Datetime )
    tag =db.Column(db.String(20))
    artefato = db.Column(db.String(10))
    valido = db.Column(db.Bolean))
